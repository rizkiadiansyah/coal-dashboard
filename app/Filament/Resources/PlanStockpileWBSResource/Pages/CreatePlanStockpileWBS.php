...
class CreatePlanStockpileWBS extends CreateRecord
{
    ...
    // PENGECEKAN DUPLIKAT: Cek apakah kombinasi material, tahun, dan bulan sudah pernah di-input
        $isExists = \App\Models\PlanStockpileWBS::query()
        ...
        PlanStockpileWBS::query()->insert($bulkData);
        ...
        return PlanStockpileWBS::query()
            ->where('material_desc', $data['material_desc'])
            ->where('tahun', $data['tahun'])
            ->where('bulan', $data['bulan'])
            ->orderBy('id', 'desc')
            ->first() ?? new PlanStockpileWBS();
    }
