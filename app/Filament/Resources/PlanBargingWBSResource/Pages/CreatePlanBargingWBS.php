...
class CreatePlanBargingWBS extends CreateRecord
{
    ...
    // PENGECEKAN DUPLIKAT: Cek apakah kombinasi material, tahun, dan bulan sudah pernah di-input
        $isExists = \App\Models\PlanBargingWBS::query()
        ...
        PlanBargingWBS::query()->insert($bulkData);
        ...
        return PlanBargingWBS::query()
            ->where('material_desc', $data['material_desc'])
            ->where('tahun', $data['tahun'])
            ->where('bulan', $data['bulan'])
            ->orderBy('id', 'desc')
            ->first() ?? new PlanBargingWBS();
    }
