...
class PlanCrushingResource extends Resource
{
    ...
    protected static ?string $navigationLabel = 'Plan Crushing';
    protected static ?string $modelLabel = 'Plan Crushing';
    protected static ?string $pluralModelLabel = 'Plan Crushing';
        ...
        return $form
            ->schema([
                Select::make('equipment')
                    ...
                    ->searchable()
                    ->preload()
                    ->options(static::getEquipmentOptions())
                    ->disabledOn('edit')
                    ->required(),
            ...
            ->filters([
                Filter::make('periode_material')
                    ...
                    ->query(function (Builder $query, array $state): Builder {
                        ...
                        if (blank($tahun) && blank($bulan) && blank($material) && blank($equipment)) {
                        ...
                        return $query
                            ...
                            ->when($equipment, fn (Builder $query) => $query->where('equipment', $equipment));
                    }),
            ])
    ...
    {
        return [
            'index' => Pages\ListPlanCrushings::route('/'),
            'create' => Pages\CreatePlanCrushing::route('/create'),
        ];
    }
    ...
    protected static function getEquipmentOptions(): array
    {
        return DB::connection('mysql_cy')
            ->table('tblcrusher')
            ->where('Status', 'Aktif')
            ->whereNotNull('Crusher')
            ->where('Crusher', '!=', '')
            ->distinct()
            ->orderBy('Crusher')
