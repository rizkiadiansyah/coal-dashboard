...
@php
    ...
    $hideIcon = $hasChildItems && trim(strip_tags($slot->toHtml())) === 'Coal In';
@endphp

<li
    x-data="{ expanded: @js(($active || $activeChildItems) && $hasChildItems) }"
    {{
        $attributes->class([
            'fi-sidebar-item',
...
>
    <a
        ...
        @class([
            ...
            'hover:bg-gray-100 focus-visible:bg-gray-100 dark:hover:bg-white/5 dark:focus-visible:bg-white/5 cursor-pointer' => filled($url) || $hasChildItems,
            'bg-gray-100 dark:bg-white/5' => $active,
        ])
    >
        @if ((! $hideIcon) && filled($icon) && ((! $subGrouped) || $sidebarCollapsible))
            <x-filament::icon
